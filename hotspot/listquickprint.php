...
$getquickprint = $API->comm("/system/script/print", array("?.id" => "$qpid"));
  ...
  $validity = $quickprintsource[9];
  $price = $quickprintsource[10];
	if($usermode == "up"){
		$tusermode =  $_user_only;
		$tchar = $charup[$char];
	}elseif($usermode == "vc"){
	...
	if (isset($_POST['name'])) {
        ...
        $source = '#'.$name.'#'.$server.'#'.$user.'#'.$userl.'#'.$prefix.'#'.$char.'#'.$profile.'#'.$timelimit.'#'.$validity.'#'.$price;
...
<table class="table">
  ...
  <tr>
	...
	<tr>
	<td class="align-middle"><?= $_validity ?></td><td><input class="form-control " type="text" size="4" autocomplete="off" name="validity" value="<?= $validity ?>"></td>
  </tr>
	<tr>
	<td class="align-middle"><?= $_price ?></td><td><input class="form-control " type="text" size="4" autocomplete="off" name="price" value="<?= $price ?>"></td>
  </tr>
	<!--<tr>
...
<?php
// get quick print
$getquickprint = $API->comm("/system/script/print", array("?comment" => "QuickPrintJuanfi"));
